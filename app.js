...
function showPhotoModal(photoId) {
  ...
  if (memoSuggest && memoInput) {
    ...
    // 기본 선택 옵션 추가 (안내글 제거하고 빈 상태 표시)
    var defaultOpt = document.createElement('option');
    defaultOpt.value = '';
    defaultOpt.textContent = '';
    if (currentMemo === '') defaultOpt.selected = true;
...
function getMemoSuggestions(targetFacilityType) {
  var counts = {};
  // window.photos 대신 파일 전역 변수 photos를 직접 참조하여 누락을 방지
  ...
  if (targetList.length > 0) {
    targetList.forEach(function (p) {
      if (p.memo && String(p.memo).trim() !== '') {
        ...
        // 일반사진용 메모와 시설물 카드용 메모 분리 필터
        var photoType = p.facilityType || '일반사진';
        var filterType = targetFacilityType || '일반사진';
  ...
  // 기존 속성 제안 정렬 방식과 동일하게 빈도순 정렬 (동일 빈도 시 가나다 순)
  list.sort(function (a, b) {
    if (b.count !== a.count) return b.count - a.count;
    return a.val.localeCompare(b.val, 'ko');
  });
...
function renderFacilityForm(container, config, cachedVals, prefixId) {
  ...
  config.fields.forEach(function (field) {
    ...
    if (field.readonly) {
      ...
      html += '<option value="">선택안함(공란)</option>';
