...
function showPhotoModal(photoId) {
  ...
  // 메모 추천 드롭다운 연동 (datalist 적용)
  var memoList = document.getElementById('photo-modal-memo-list');
  var memoSuggest = document.getElementById('photo-modal-memo-suggest');
  if (memoSuggest) memoSuggest.style.display = 'none';
  ...
  if (memoList) {
    ...
    memoSuggs.forEach(function (sug) {
      ...
      opt.value = sug;
      memoList.appendChild(opt);
    });
  }

  // 메모 입력 시 실시간 미리보기 업데이트 트리거
...
function showStreetlightInputForm(fileBlob, item, dxfCoords, latLng) {
  ...
  // 사진 메모 입력 필드 (공통 - 입력창 + 추천 선택 datalist 통합)
  ...
  var memoHtml = '<label>메모 (사진메모)</label>' +
                 '<input type="text" id="sw-form-memo" list="sw-form-memo-list" value="" placeholder="예: 메모 입력 또는 선택">';
  
  memoGroup.innerHTML = memoHtml;
  
  var datalist = document.createElement('datalist');
  datalist.id = 'sw-form-memo-list';
  memoSuggestions.forEach(function (sug) {
    var opt = document.createElement('option');
    opt.value = sug;
    datalist.appendChild(opt);
  });
  memoGroup.appendChild(datalist);
...
function saveStreetlightData(formData, fileBlob, item, dxfCoords, latLng) {
  ...
  function finishSave(blob) {
    ...
    var photo = {
      ...
      height: 1,
      blob: blob,
      memo: formData.memo || (descText + ' (' + item.name + ')'),
...
function renderFacilityForm(container, config, cachedVals, prefixId) {
  ...
  config.fields.forEach(function (field) {
    ...
    if (field.readonly) {
      ...
      var placeholder = field.placeholder || (isNumericField ? '숫자 입력' : '직접 입력 또는 선택');

      html += '<input type="' + inputType + '"' + stepAttr + ' id="' + inputId + '" list="' + listId + '" value="' + val + '" placeholder="' + placeholder + '">';
      group.innerHTML = html;

      // datalist 생성 및 옵션 바인딩
      var datalist = document.createElement('datalist');
      datalist.id = listId;
