...
(function () {
  ...
  function onOverlayAdd() {
    ...
    // 썸네일 카드 내에 도면 전개될 사진 메모가 존재한다면 초록색으로 표기
    if (this.photo.memo && !isAutoGeneratedMemo(this.photo.memo)) {
      var memoEl = document.createElement('div');
      memoEl.className = 'photo-overlay-memo';
      memoEl.style.color = '#34C759'; // 초록색
      memoEl.style.fontSize = '11px';
      memoEl.style.fontWeight = 'bold';
      memoEl.style.marginTop = '2px';
      memoEl.textContent = this.photo.memo;
      div.appendChild(memoEl);
    }

    // [고도화 추가] 지도 상에 정보 텍스트(속성, 메모) 줄바꿈 상시 노출 패널 부착 (저장 완료된 사진은 숨김)
    ...
    if (metadata && metadata.texts) {
      ...
      ids.forEach(function (id) {
        ...
        if (tObj && tObj.text) {
          ...
          if (tObj.layer === '사진번호' || tObj.layer === '사진메모') {
            return;
          }
          if (isAutoGeneratedMemo(tObj.text)) {
            return;
          }
    ...
    overlays.forEach(function (o) {
      if (o.div) {
        ...
        if (isSaved && !(map && map.getZoom() <= 17 && o.isClusterRepresentation)) {
          if (o.div) {
            o.div.classList.remove('text-only-mode');
        ...
        if (isLabelShow && o.isMultiple && o.photo !== o.overlaps[0]) {
          if (o.div) o.div.style.display = 'none';
          return;
        } else {
          if (!(map && map.getZoom() <= 17 && !o.isClusterRepresentation)) {
            if (o.div) o.div.style.display = 'block';
          }
        }
  ...
  function detectFacilityType(fileName, memo) {
    ...
    if (fn.indexOf('자전거') >= 0 || m.indexOf('자전거') >= 0) {
      return null;
    }

    // 1) 사진 메모나 파일명이 FACILITY_CONFIG의 키와 완전 일치하는지 우선 검사
    if (m && FACILITY_CONFIG[m]) {
      return m;
    }
    if (fn && FACILITY_CONFIG[fn]) {
      return fn;
    }

    // 2) 메모나 파일명 대소문자 무관 완전 일치 검사 폴백
    if (m) {
    ...
    if (fn) {
      for (var key in FACILITY_CONFIG) {
        if (key.toLowerCase() === fn.toLowerCase()) {
          return key;
        }
      }
    }
  ...
  function updateProgressBar() {
    ...
    var completed = metadata.photos.filter(function (p) { return p.edited === true; }).length;
    var pct = total > 0 ? Math.round((completed / total) * 100) : 0;

    var percentEl = document.getElementById('progress-percent');
    if (percentEl) {
      percentEl.textContent = pct + '% (' + completed + '/' + total + '개 완료)';
    }
