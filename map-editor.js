    ...
    if (metadata && metadata.texts) {
      ...
      ids.forEach(function (id) {
        ...
        if (tObj && tObj.text) {
          ...
          var txt = tObj.text.trim();
          var slashIdx = txt.indexOf('/');
          if (slashIdx >= 0) {
            var prefix = txt.substring(0, slashIdx);
            if (FACILITY_CONFIG[prefix] !== undefined) {
              txt = txt.substring(slashIdx + 1);
            }
          }
          specTexts.push(txt);
        }
  ...
  function getPhotoSummaryText(photo) {
    ...
    if (photo.facilityType) {
      if (photo.specTextIds && photo.specTextIds.length > 1) {
        return '속성 외 ' + (photo.specTextIds.length - 1) + '건';
      }
